...
export async function Config(server_id: string) {
	// check id server
	if (server_id) {
...
export async function GM(server_id: any, uid: any, cmd: any, code: any) {
	try {
		...
		// check server
		var configis = await Config(server_id)
...
export async function Server(server_id: string = "") {
...
export async function SH(raw: string, server_id: string, timeout: number = 30) {
...
export async function Verified(tes: any) {
	/*
			   if (!set_code) {
				   // find id user in key
